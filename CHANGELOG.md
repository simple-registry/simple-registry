...
- A `[repository]` declaring `namespace = "docker.io"` serves pulls naming it in the proxy `?ns=` parameter, so a mirroring client such as containerd reaches the cache without prefixing the paths it requests, and the response echoes `OCI-Namespace`. An `ns` no repository declares, and `ns` on a write, are ignored as before.
...
- **Breaking:** 4XX error bodies now carry only the OCI codes the spec defines. `BAD_REQUEST` is emitted as `UNSUPPORTED`, `NOT_FOUND` as `NAME_UNKNOWN`, `RANGE_NOT_SATISFIABLE` as `SIZE_INVALID`, and the immutable-tag or concurrent-write `CONFLICT` as `DENIED`; each keeps its message and HTTP status. `REPLICATION_SUPERSEDED` stays as it is, answering replication writes only. A client matching on the previous codes must update.
