...
A blob-index entry names the manifest that references the blob, so the shard alone records which manifests still hold it and no per-layer or per-config link file is written. A link file left by an angos that wrote them is migrated by `angos scrub`, which re-issues the references it holds as entries and then reclaims the file, so a registry converges on the new shape without an operator step.

Blob-index shards are the one hot key that many concurrent pushes contend on, so their mutations are idempotent set-merges carrying an add/remove delta rather than a whole-body overwrite. A merge carries no read: it re-reads the live shard and re-applies its delta at apply time and again on replay, so a racing write costs it one re-read rather than aborting the transaction, and two pushes stop contending over the base layers their images share. A shard whose bytes a transaction decided on, the reclaim path's unreferenced check and a strict push's reference-ownership check, is read-set validated so a racing write fails prepare and the decision is re-taken. The `blob-data:{digest}` coarse lock continues to isolate the delete path's unreferenced check.
