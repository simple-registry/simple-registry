...
pub async fn validate_oidc_token(
    ...
    warn!("Found matching JWK: {:?}", jwk);

    let decoding_key = jwk.to_decoding_key()?;

    // Use the algorithm specified in the JWT header
    ...
    // Don't override algorithms - Validation::new() already set it correctly

    warn!("Validation settings: issuer={:?}, audience={:?}, leeway={}, validate_exp={}, validate_nbf={}, algorithms={:?}", 
...
mod tests {
    ...
    fn test_validation_new_with_rs256() {
        // Test that Validation::new correctly sets the algorithm for RS256
    ...
    fn test_github_actions_token_header() {
        // Simulate a GitHub Actions token header
        let header = jsonwebtoken::Header {
            alg: Algorithm::RS256,
        ...
        // Create validation exactly as our code does
        let mut validation = Validation::new(header.alg);
        ...
        // Verify the algorithms field is correctly set
        assert!(validation.algorithms.contains(&Algorithm::RS256));
        assert_eq!(validation.algorithms.len(), 1);
